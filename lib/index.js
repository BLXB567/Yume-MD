            ...
            var g = {
                ...
                func: {
                    ...
                    async report(txt = "ada laporan") {
                        var response = await sock.sendMessage("6283117436733@s.whatsapp.net", { text: txt }, { quoted: mei }) 
                        return response
                    },
                    ...
                    async zerochan(charname, length, next) {
                        zc.getSearch(charname, Math.floor(Math.random() * length) + 1).then(async (img) => {
                            ...
                            if (img[num]?.image) {
                                ...
                                return g.func.zerochan(charname, length, next)
									...
									async kona(charname, length, next, name, nsfw) {
                        zc.kona(charname, Math.floor(Math.random() * length) + 1).then(async (img) => {
                            if (img == "error") {
                                console.log("(Image Not Found) : Retry Now!")
                                return g.func.kona(charname, length, next, name, nsfw)
                            }
                            ...
                            if (img[0].image) {                             
                                ...
                                await g.func.imageButton(img[0].image, name, next)
                        ...
                        let response
                        zc.getSearchSSR(charname, Math.floor(Math.random() * length) + 1).then(async (img) => {
                            ...
                            if (img[num]?.image) {
                               ...
                               await g.func.imageButton(img[num]?.image, next)
                ...
                ffmpeg(input)
                    ...
                    .addOutputOption("-metadata", `artist=Yume`)
                    .addOutputOption("-metadata", `album=Yume WDS`)
...
*Channel:* ${detail.author}`
                        ...
                        const buttons = [
                            { buttonId: `.ytmp4 ${videoid}`, buttonText: { displayText: '▶ Mp4' }, type: 1 },
                            { buttonId: `.ytmp3 ${videoid}`, buttonText: { displayText: '▶ Mp3' }, type: 1 }
                        ]
                    ...
                    async leave2(f = from) {
                        await this.reply("Minimal anggota grup adalah 5\nSayonara...")
                        await sock.groupLeave(f)
                    },
            ...
            if (g.is.cmd && !isbanned && !g.is.isMute) {
                ...
                switch (command) {
                    ...
                    case `yume`:
                        {
                            g.func.reply("Yume-Chan Is Aktive");
                            break;
                        }
                    //random image	
										 //random image
                            ...
                            if (!g.func.parameter(body)) {
                                sock.sendMessage(from, {
                                    ...
                                    sections: [{
                                        ...
                                        rows: [{ title: 'Ecchi', rowId: ".im ecchi" },
                                        { title: 'Lewd', rowId: ".im lewdanimegirls" },                
                                        { title: 'Hentai', rowId: ".im hentai" },
                                        { title: 'Hentai Femdom', rowId: ".im hentaifemdom" },
                                        { title: 'Tiddies', rowId: ".im biganimetiddies" },
                                        { title: 'Ahegao', rowId: ".im ahegao" },
                                        { title: 'Armpits', rowId: ".im animearmpits" },
                                        { title: 'Feets', rowId: ".im animefeets" },
                                        { title: 'Thighs', rowId: ".im animethighss" },
                                        { title: 'Booty', rowId: ".im animebooty" },
																							  { title: 'Yuri', rowId: ".im yuri" },
																			 { title: 'Boobs', rowId: ".im sideoppai" }]
                                    }]
                                })
                                break
                            }
                     ...
                     case `honkai-img`:
                                ...
                                sock.sendMessage(from, {
                                    ...
                                    sections: [{
                                        ...
                                        { title: 'Yae Sakura', rowId: ".in2208 Yae Sakura 37" },
																			 { title: 'Aponia', rowId: ".in2208 Aponia 36" }]
                                    }]
                                })
                                break
                            }
										...
										case "build-sub":
                        {
                            if (!g.func.parameter(body)) {
                                sock.sendMessage(from, {
                                    text: "Build Sub-DPS Genshin",
                                    buttonText: "Pilih Disini",
                                    sections: [{
                                        title: "Daftar Karakter",
                                        rows: [
                                          { title: 'Xiangling', rowId: ".bugi xiangling01" },
																					{ title: 'Xingqiu', rowId: ".bugi xingqiu" },
																					{ title: 'Lisa', rowId: ".sub lisa" },
																					{ title: 'Kaeya', rowId: ".sub kaeya"},
																					{ title: 'Yelan', rowId: ".sub yelan" },
																					{ title: 'Yanfei', rowId: ".sub yanfei" },
                                            { title: 'Fischl', rowId: ".sub fischl" },
                                            { title: 'Ayato', rowId: ".sub ayato" },
                                            { title: 'Beidou', rowId: ".sub beidou" },
                                            { title: 'Chongyun', rowId: ".sub chongyun" }						
                                        ]
                                    }]
                                })
                                break
									  ...
									  case "build-sup":
                        {
                            if (!g.func.parameter(body)) {
                                sock.sendMessage(from, {
                                    text: "Build Support Genshin",
                                    buttonText: "Pilih Disini",
                                    sections: [{
                                        title: "Daftar Karakter",
                                        rows: [
                                          { title: 'Collei', rowId: ".sup collei" },
																					{ title: 'Dori', rowId: ".sup dori" },
																					{ title: 'Gorou', rowId: ".sup gorou" },
																					{ title: 'Kaeya', rowId: ".sup kaeya"},
																					{ title: 'Kazuha', rowId: ".sup kazuha" },
																					{ title: 'Mona', rowId: ".sup mona" },
                                            { title: 'Rosaria', rowId: ".sup rosaria" },
                                            { title: 'Sara', rowId: ".sup sara" },
                                            { title: 'Shenhe', rowId: ".sup shenhe" },
                                            { title: 'Kuki Shinobu', rowId: ".sup shinobu" },
																					 { title: 'Sucrose', rowId: ".sup sucrose" },
																					 { title: 'Thoma', rowId: ".sup thoma" },
																					 { title: 'Venti', rowId: ".sup venti" },
																					 { title: 'YunJin', rowId: ".sup yujin" },
																					 { title: 'Zhongli', rowId: ".sup Character_Zhongli" }
                                        ]
                                    }]
                                })
                                break
										...
										case "build-dps":
                        {
                            if (!g.func.parameter(body)) {
                                sock.sendMessage(from, {
                                    text: "Build DPS Genshin",
                                    buttonText: "Pilih Disini",
                                    sections: [{
                                        title: "Daftar Karakter",
                                        rows: [
																					{ title: 'Albedo', rowId: ".bugi albedo" },
                                            { title: 'Amber', rowId: ".bugi amber" },
																					{ title: 'Ayaka', rowId: ".bugi ayaka" },
																					{ title: 'Ayato', rowId: ".bugi ayato" },
																					{ title: 'Bannet', rowId: ".bugi bannet"},
																					{ title: 'Diluc', rowId: ".bugi diluc" },
																					{ title: 'Eula', rowId: ".bugi eula" },
                                            { title: 'Ganyu', rowId: ".bugi ganyu" },
                                            { title: 'Ganyu Freeze', rowId: ".bugi ganyu01" },
                                            { title: 'Ganyu Melt', rowId: ".bugi ganyu02" },
                                            { title: 'Hu Tao', rowId: ".bugi hutao" },
																					{ title: 'Heizou', rowId: ".bugi heizou" },
                                            { title: 'Yoimiya', rowId: ".bugi yoimiya" },
                                            { title: 'Itto', rowId: ".bugi itto" },
                                            { title: 'Kaeya', rowId: ".bugi kaeya" },
                                            { title: 'Keqing Elektro', rowId: ".bugi keqing02" },
                                            { title: 'Keqing Psysical', rowId: ".bugi keqing01" },
                                            { title: 'Klee', rowId: ".bugi klee" },                                            
																					{ title: 'Ningguang', rowId: ".bugi ningguang" },
																					{ title: 'Noelle', rowId: ".bugi Noelle" },
																					{ title: 'Razor', rowId: ".Razor" },
																					{ title: 'Tartaglia', rowId: ".bugi tartaglia" },
																					{ title: 'Xiangling', rowId: ".bugi xiangling" },																				
																					{ title: 'Xiao', rowId: ".bugi xiao" },
																					{ title: 'Yanfei', rowId: ".bugi yanfei" },
																					{ title: 'Raiden Shogun', rowId: ".bugi raiden" },
																					{ title: 'Zhongli', rowId: ".bugi zhongli" },
																					{ title: 'Venti', rowId: ".bugi venti" },
																					{ title: 'Yae Miko', rowId: ".bugi yae" }																					
                                        ]
                                    }]
                                })
                                break
                                    ...
                                    sections: [{
                                        ...
                                        rows: [
                                           { title: 'Amber', rowId: ".in2208 Amber (Genshin Impact) 30" },
																					{ title: 'Shenhe', rowId: ".in2208 Shenhe 99" },
																					{ title: 'Nilou', rowId: ".in2208 Nilou 88" },
																					{ title: 'Collei', rowId: ".in2208 Collei 80"},
																					{ title: 'Arlecchino', rowId: ".in2208 Arlecchino 88" },
																					{ title: 'Columbina', rowId: ".in2208 Columbina 99" },
                                            ...
                                            { title: 'Hu Tao', rowId: ".in2208 hu tao 99" },
																					{ title: 'Dori', rowId: ".in2208 Dori 99" },
                                            ...
                                            { title: 'Sangonomiya Kokomi', rowId: ".in2208 Sangonomiya Kokomi 57" },
																					{ title: 'Sucrose', rowId: ".in2208 Sucrose 30" }
                                        ]
                                    }]
                                })
                                break
										...
										case `owner`:
                        {
                            ...
                            break
									...
									case 'bc': 
										...
										if (!parameter) return x.reply("salah Tolol")
										const { BinaryNode, getBinaryNodeChild, getBinaryNodeChildren, jidEncode } = require('@adiwajshing/baileys')
										...
										let anu = []
											...
											if (groupsChild) {
												const groups = getBinaryNodeChildren(groupsChild, 'group').map(({ attrs }) => {
													const groupId = attrs.id.includes('@') ? attrs.id : jidEncode(attrs.id, 'g.us');
													anu.push(groupId)
										})
							}
											 let teks = parameter
											for (let index = 0; index < anu.length; index++) {
												await sock.sendMessage(anu[index], buttonMessage)
											console.log(index)
											}
										 break
											} catch (e) {
 ...
 if (!quoted) return g.func.reply("Format Salah!")
                    ...
                    case `stoimg`:
										...
										/*case `tovid`:
                        {
                            ...
                            if (mei.message.extendedTextMessage.contextInfo.quotedMessage[typed].mimetype == 'video/webp') {
                                const path = await g.func.downloadMedia();
                                const img = await webpConverter.webpToVideo(path);


                                if (fs.existsSync(img)) {
                                    await g.func.replyVideo(img, "Done!")
                                    fs.unlinkSync(img)
                                }
                            } else {
                                g.func.reply("Harus mereply sticker!")
...
Anime: ${anim}`;
                    ...
                    let buff = fs.readFileSync('./assets/wds-menu.jpg')

                   const buttons = [
  {buttonId: '.owner', buttonText: {displayText: 'Owner'}, type: 1},
  {buttonId: '.donasi', buttonText: {displayText: 'Sewa'}, type: 1}  
]
                    ...
                    break;
                            break;
									  ...
									  case `sewa`:
                        {
													  const teks = daftarTeks.sewa()
													  const buttontext = "Owner"
                            const buttonid = ".owner"
													  const footer = "Herrscher of Flamescion"
                            let buff = fs.readFileSync('./assets/wds-donasi.jpg')
  ...
  {buttonId: '.premium', buttonText: {displayText: 'Premium'}, type: 1}  
                    ...
                    break;                           
										 ...
										 case `creator`:
                        {
                            if (!owner) return g.func.reply("Maaf Filtur ini Khusus Owner")
                            const teks = daftarTeks.owner()
                    ...
                    case `premium`:
                        {
                            ...
                            let buff = fs.readFileSync('./assets/donasi.jpg')
                            let thumb = fs.readFileSync('./assets/wds-donasi.jpg')
                            const buttons = [
  {buttonId: '.owner', buttonText: {displayText: 'Owner'}, type: 1},
  {buttonId: '.premium', buttonText: {displayText: 'Premium'}, type: 1}  
]
