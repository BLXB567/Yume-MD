...
const username = "Syah WDS"
const pass = "PROGIB51"
const username2 = "syah5520"
const pass2 = "PROGIB5123"
...
//===========================================================================
  ...
  //=========================================================================
  //Danbooru
  dan: async function(keyword, page = 1, options) {
  ...
  //=========================================================================
 kona: async function (keyword, page = 1, options) {
		return await fetch(`https://konachan.com/post.json?page=${page}&tags=${keyword}`)
			.then(response => response.json())
			.then(img => {
				const num = Math.floor(Math.random() * 19) + 1
				const result = []
	...
	//=========================================================================	
  //Konachan: Safe Mode
	danbo: async function(keyword, page = 1, options) {
		
    if (isNaN(page)) page = 1;

    const obj = {
      uri: `https://danbooru.donmai.us/${parseInput(keyword)}?p=${page}`,
      transform: function(body) {
        return cheerio.load(body);
      }
    };
     ...
     if (obj.uri.includes('shuushuu')) {
      return await rp(obj) /* eslint no-return-await: 0 */
        .then($ => {
          ...
          $('.thumb').each(function() { /* eslint func-names: 0 */
            results.push({
              image: "https://danbooru.donmai.us" + $(this).children('a')
                .first()
                .attr('href')
            });
          });
